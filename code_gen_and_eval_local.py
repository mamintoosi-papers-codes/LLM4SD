...
def auto_gen_code(prompt):
    ...
    while True:
        try:
            response = ollama.chat(model=OLLAMA_CODE_GEN_MODEL, messages=[{"role": "user", "content": prompt}])
            generated_code = response["message"]["content"]
            break  # If successful, exit loop
        except Exception as e:
            print(f"An error occurred: {str(e)} - Retrying in 2 seconds...")
            time.sleep(2)

    # Extract only function definitions from generated code
...
def get_inference_file_path():
    ...
    # M. Amintoosi
    inference_file_name = f"{args.model}_summarized_rules.txt"
